...
class csi_data_graphical_window(QWidget):
    def __init__(self):
        ...
        self.plotWidget_amplitude = PlotWidget(self)
        self.plotWidget_amplitude.setGeometry(QtCore.QRect(0, 0, 640, 720))
        self.plotWidget_amplitude.setYRange(-20, 100)
        self.plotWidget_amplitude.addLegend()

        self.plotWidget_phase = PlotWidget(self)
        self.plotWidget_phase.setGeometry(QtCore.QRect(640, 0, 640, 720))
        self.plotWidget_phase.setYRange(-np.pi, np.pi)
        ...
        self.curve_list_amplitude = []
        self.curve_list_phase = []

        for i in range(CSI_DATA_COLUMNS):
            curve_amplitude = self.plotWidget_amplitude.plot(
                self.csi_amplitude_array[:, i], name=f'Amplitude {i}', pen=csi_vaid_subcarrier_color[i])
            self.curve_list_amplitude.append(curve_amplitude)

            curve_phase = self.plotWidget_phase.plot(
                self.csi_phase_array[:, i], name=f'Phase {i}', pen=csi_vaid_subcarrier_color[i])
            self.curve_list_phase.append(curve_phase)
    ...
    def update_data(self):
        ...
        for i in range(CSI_DATA_COLUMNS):
            self.curve_list_amplitude[i].setData(self.csi_amplitude_array[:, i])
            self.curve_list_phase[i].setData(self.csi_phase_array[:, i])
